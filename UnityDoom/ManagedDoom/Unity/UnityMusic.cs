...
namespace ManagedDoom.Unity
{
    public sealed class UnityMusic : IMusic, IDisposable
    {
        ...
        public void OnGetData(float[] samples)
        {
            if (stream == null) return;

            if (stream.Reserved != stream.Current)
            {
                stream.SynthesizerInstance.Reset();
                stream.Current = stream.Reserved;
            }
            ...
            stream.Current.RenderWaveform(stream.SynthesizerInstance, left, right);
        ...
        private class MusStream : IDisposable
        {
            ...
            public Synthesizer SynthesizerInstance => synthesizer;
            public IDecoder Current { get => current; set => current = value; }
            public IDecoder Reserved { get => reserved; set => reserved = value; }
            ...
            public void Play()
            {
                if (!audioClip)
                {
                    // DelegateSupport converts the C# Action into an IL2CPP PCMReaderCallback delegate pointer
                    var pcmCallback = DelegateSupport.ConvertDelegate<AudioClip.PCMReaderCallback>(new Action<float[]>(parent.OnGetData));

                    audioClip = AudioClip.Create(
                        "Doom_Music",
                        MusDecoder.SampleRate,
                        2,
                        MusDecoder.SampleRate,
                        true,
                        pcmCallback
